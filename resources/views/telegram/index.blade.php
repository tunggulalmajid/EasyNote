<x-app-layout>
    ...
    {{-- ROOT ALPINE JS --}}
    {{-- Container: Padding disesuaikan (py-6 di HP) --}}
    <div class="max-w-2xl mx-auto py-6 px-4 sm:px-6 lg:px-8 sm:py-10" x-data="telegramApp('{{ $telegram ?? '' }}')">

        <div class="bg-neutral-800 border border-neutral-700 rounded-xl shadow-lg overflow-hidden">

            {{-- Header Card --}}
            <div class="p-5 sm:p-6 border-b border-neutral-700 bg-neutral-800/50">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <h2 class="text-lg sm:text-xl font-bold text-white">Notifikasi Telegram</h2>
                </div>
            ...
            <div class="p-5 sm:p-6 space-y-6 sm:space-y-8">
                ...
                {{-- Tutorial Section --}}
                <div
                    class="bg-neutral-900/50 rounded-xl p-4 sm:p-5 border border-neutral-700/50 text-sm text-neutral-300">
                    ...
                    <ol class="list-decimal list-inside space-y-2 text-neutral-400 ml-1">
                        ...
                        <li>Cari bot kami: <a href="https://t.me/OnEasyNoteBot"
                                class="text-blue-400 font-medium cursor-pointer hover:underline break-all">@EasyNote</a>
                        </li>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        {{-- Responsive Input Group: Column di HP, Row di Desktop --}}
                        <div class="flex flex-col sm:flex-row gap-3">
                            {{-- Input Field --}}
                            <input type="text" x-model="chatId" id="telegram" placeholder="Contoh: 123456789"
                                class="w-full sm:flex-1 rounded-lg bg-neutral-900 border-neutral-700 text-white placeholder-neutral-600 focus:ring-blue-500 focus:border-blue-500 shadow-sm transition disabled:opacity-50 p-2.5"
                                :disabled="isLoading">

                            {{-- Tombol Simpan --}}
                            <button @click="saveId()"
                                class="w-full sm:w-auto justify-center px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-medium transition flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-blue-900/20 active:scale-95"
                                :disabled="isLoading">
                    ...
                    <div x-show="isConnected" x-transition:enter="transition ease-out duration-300"
                        ...
                        class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20 mt-4">

                        <div class="flex items-center gap-3">
                            <div class="p-1.5 rounded-full bg-emerald-500/20 text-emerald-400 shrink-0">
                        ...
                        <button @click="disconnect()"
                            class="text-sm text-red-400 hover:text-red-300 hover:underline decoration-red-400/30 transition flex items-center gap-1 self-start sm:self-auto"
                            :disabled="isLoading">
    ...
    {{-- Script AlpineJS (Logic tetap sama) --}}
    <script>
        function telegramApp(initialId) {
            return {
                ...
                get isConnected() {
                    return this.chatId && this.chatId.length > 5;
                },

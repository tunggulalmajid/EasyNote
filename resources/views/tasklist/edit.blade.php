        ...
        <div class="bg-neutral-800 border border-neutral-700 rounded-xl shadow-lg p-6 sm:p-8">
            ...
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-white flex items-center gap-2">
                    <i data-lucide="edit-3" class="w-6 h-6 text-amber-400"></i>
                    Edit Tugas
                </h2>
                <p class="text-neutral-400 text-sm mt-1">Perbarui informasi tugas Anda.</p>
            </div>
            ...
            <form action="{{ route('task.update', $task->id) }}" method="POST" class="space-y-6">
                ...
                {{-- Input Deskripsi --}}
                <div>
                    <label for="deskripsi" class="block text-sm font-medium text-neutral-300 mb-1">Deskripsi</label>
                    <textarea name="deskripsi" id="deskripsi" rows="3"
                        class="w-full rounded-lg bg-neutral-900 border-neutral-700 text-white placeholder-neutral-500 focus:ring-indigo-500 focus:border-indigo-500 shadow-sm">{{ old('deskripsi', $task->deskripsi) }}</textarea>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {{-- Input Kategori --}}
                    <div>
                        <label for="category_id" class="block text-sm font-medium text-neutral-300 mb-1">Kategori</label>
                    ...
                    {{-- Input Status --}}
                    <div>
                        <label for="status_id" class="block text-sm font-medium text-neutral-300 mb-1">Status</label>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="deadline" id="deadline"
                        value="{{ old('deadline', \Carbon\Carbon::parse($task->deadline)->format('Y-m-d\TH:i')) }}" required
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-700">
                    <a href="{{ route('task.index') }}" class="px-4 py-2 text-sm font-medium text-neutral-300 bg-neutral-800 border border-neutral-600 rounded-lg hover:bg-neutral-700 transition">
                        Batal
                    </a>
                    <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-500 shadow-lg shadow-indigo-900/20 transition flex items-center gap-2">
